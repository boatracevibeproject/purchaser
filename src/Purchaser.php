 ...
 * テレボートを操作して舟券を買う本体。
 *
 * 静的に呼ぶためのシングルトンも同居させている。組織内で最も新しい scraper は
 * 静的シングルトンを明示的に持たないが、その理由は「インスタンスごとに pacing と
 * cache を分けたい」からで、purchaser にその要求は無い。テレボートのセッションは
 * ファイルロックでプロセスをまたいで直列化しているので、インスタンスを増やしても
 * 同時に動かせるのは1つだけになる。
 *
 * DI コンテナに載せるなら、この静的な入口を使わずに Contracts\Purchaser を
 * このクラスへ束ねて注入すればよい。
 *
 * @author shimomo
 */
final class Purchaser implements Contracts\Purchaser
{
     ...
     */
    #[\Override]
    public function setMaxDepositAmount(int $maxDepositAmount): Purchaser
    {
     ...
     */
    #[\Override]
    public function setMaxTotalAmount(int $maxTotalAmount): Purchaser
    {
     ...
     */
    #[\Override]
    public function setMaxBatchAmount(?int $maxBatchAmount): Purchaser
    {
     ...
     */
    #[\Override]
    public function setSubscriberNumber(#[\SensitiveParameter] string $subscriberNumber): Purchaser
    {
     ...
     */
    #[\Override]
    public function setAuthenticationPassword(#[\SensitiveParameter] string $authenticationPassword): Purchaser
    {
     ...
     */
    #[\Override]
    public function setPurchasePassword(#[\SensitiveParameter] string $purchasePassword): Purchaser
    {
     ...
     */
    #[\Override]
    public function setDeadline(?DateTimeInterface $deadline, int $marginSeconds = 5): Purchaser
    {
     ...
     */
    #[\Override]
    public function setArtifactDirectory(?string $artifactDirectory): Purchaser
    {
     ...
     */
    #[\Override]
    public function setLockPath(string $lockPath): Purchaser
    {
     ...
     * 既にあればそれを返し、無ければ作る。
     *
     * @param ?\BVP\Purchaser\Contracts\Purchaser $purchaser
     * @param ?non-empty-string $seleniumServerUrl
     * @param ?int<1000, max> $maxDepositAmount
     * @param ?non-empty-string $subscriberNumber
     * @param ?non-empty-string $personalIdentificationNumber
    ...
    public static function instance(
        ?Contracts\Purchaser $purchaser = null,
        ?string $seleniumServerUrl = null,
        ?int $maxDepositAmount = self::DEFAULT_MAX_DEPOSIT_AMOUNT,
        ...
        return self::$instance ??= $purchaser ?? new self(
            $seleniumServerUrl,
            $maxDepositAmount,
            $subscriberNumber,
